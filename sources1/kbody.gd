...
#	motion = Vector2(8, 8)
	...
	get_parent().get_node("accel-label").set_text(str(round(angle_deg)))
#	print ("before: " + str(motion.x))

	var ball_deg = atan(motion.x/motion.y) * (180/PI)
	get_parent().get_node("ball-angle-label").set_text(str((ball_deg)))
